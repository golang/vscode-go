...
export class GoDebugSession extends LoggingDebugSession {
	 ...
	 */
	protected async inferLocalPathFromRemotePath(remotePath: string): Promise<string | undefined> {
		...
		const potentialMatches = await this.findFilesInWorkspace(fileName, this.delve.program);

		const bestMatchingLocalPath = this.findPathWithBestMatchingSuffix(remotePath, potentialMatches);
	...
	/**
	 * Find files in the workspace by first calling the VSCode findFiles method (Fast, indexed).
	 * In case that fails, fall back to glob, but async so we can paralellize the calls.
	 * @param fileName The name of the file to search
	 * @param cwd The current working directory
	 * @returns List of relative paths matching the filename
	 */
	protected async findFilesInWorkspace(fileName: string, cwd: string): Promise<string[]> {
		try {
			const { files } = await this.callExtensionHost<{ path: string; reqId: string; }, { files: string[]; }>('findFiles', {
				path: `**/${fileName}`,
				reqId: `findFiles:${fileName}`
			});
			...
			return await this.globAsync(fileName, {
				matchBase: true,
	 ...
	 */
	protected async toLocalPath(pathToConvert: string): Promise<string> {
		if (this.substitutePath.length === 0) {
			// User trusts use to infer the path
			if (this.delve.isRemoteDebugging) {
				const inferredPath = await this.inferLocalPathFromRemotePath(pathToConvert);
				if (inferredPath) {
					return inferredPath;
				}
			}
	...
	protected async stackTraceRequest(
		...
		this.delve.call<DebugLocation[] | StacktraceOut>(
			...
			async (err, out) => {
				...
				let stackFrames = await Promise.all(locations.map(async (location, frameId) => {
					const uniqueStackFrameId = this.stackFrameHandles.create([goroutineId, frameId]);
					const localPath = location.file !== '<autogenerated>' ? await this.toLocalPath(location.file) : null;
					return new StackFrame(
						uniqueStackFrameId,
						location.function ? location.function.name : '<unknown>',
						location.file === '<autogenerated>'
							? null
							: new Source(path.basename(location.file), localPath),
						location.line,
						0
					);
				}));
	...
	/**
	 * Works in conjunction with `callExtensionHost` to handle responses to the call.
	 */
	protected async customRequest(command: string, response: DebugProtocol.Response, args: any, request?: DebugProtocol.Request) {
		if (this.requestMap.has(command)) {
			const resolveArr = this.requestMap.get(command);
			resolveArr.forEach(resolve => resolve(args));
		}
	...
	/**
	 * Calls the Extension Host (VSCode) in a reverse way to request information.
	 * Since no reverse IPC exists, this is implemented as an event to the clientm
	 * to which the client responds with a custom request, which is handled by the
	 * custom request handler.
	 * @param method Name of the implemented method to call in the Extension Host
	 * @param args Typed arguments for the call
	 * @param timeoutMs How quickly the call should time out
	 * @returns A typed response from the Extension Host
	 */
	private async callExtensionHost<TReq extends { reqId: string }, TRes>(method: string, args: TReq, timeoutMs = 30000): Promise<TRes> {
		let requestMapId = args.reqId;

		log('Sending "' + requestMapId + '" to extension host', args);

		return new Promise<TRes>((resolve, reject) => {
			let timeoutId: NodeJS.Timeout;
			const resHandl = (data: TRes) => {
				log('Got response for "' + requestMapId + '"', 'data:', data);
				if (timeoutId) {
					clearTimeout(timeoutId);
				}
				this.requestMap.delete(requestMapId);

				resolve(data);
			};
			if (this.requestMap.has(requestMapId)) {
				this.requestMap.set(requestMapId, [...this.requestMap.get(requestMapId), resHandl]);
			} else {
				this.requestMap.set(requestMapId, [resHandl]);
			...
			timeoutId = setTimeout(() => {
				reject(new Error('Timout exceeded calling VSCode'));
				this.requestMap.delete(requestMapId);
			}, timeoutMs);
		});
	}

	// Calls glob as a Promise instead of sync.
	private async globAsync(fileName: string, args: any): Promise<string[]> {
		return new Promise<string[]>((resolve, reject) => {
			glob(fileName, args, (err, matches) => {
				if (err) {
					return reject(err);
				}
	...
	private async getPackageInfo(debugState: DebuggerState): Promise<string | void> {
		...
		const localPath = await this.toLocalPath(debugState.currentThread.file);
