...
export class Delve {
	...
	private goModName: string | undefined = undefined;
...
export class GoDebugSession extends LoggingDebugSession {
	...
	private localGoModNameMapping = new Map<string, string>();
	private remoteGoModName: string | undefined = undefined;
	 ...
	 */
	protected async inferRemotePathFromLocalPath(localPath: string): Promise<string | undefined> {
	...
	protected async toDebuggerPath(filePath: string): Promise<string|undefined> {
		if (this.delve.remotePath.length === 0) {
			if (this.delve.isRemoteDebugging) {
				...
				const matchedRemoteFile = await this.inferRemotePathFromLocalPath(filePath);
				...
				let stackFrames = await Promise.all(locations.map(async (location, frameId) => {
					...
					return new StackFrame(
						...
						location.file === '<autogenerated>'
							? null
							: new Source(path.basename(location.file), await this.toLocalPath(location.file)),
						location.line,
						0
					);
				}));
	...
	/**
	 * Given a local path, attempt to retrieve the module that path is in.
	 * This is cached in localGoModNameMapping dictionary.
	 */
	private async getLocalGoModName(localPath: string): Promise<string> {
		const dirName = path.dirname(localPath);
		if (this.localGoModNameMapping.has(dirName)) {
			return this.localGoModNameMapping.get(dirName);
		}

		try {
			const localGoModName: string|undefined = await new Promise((resolve) => {
				execFile(
					getBinPathWithPreferredGopathGoroot('go', []),
					['list', '-m'],
					{ cwd: dirName, env: this.delve.dlvEnv },
					(err, stdout, stderr) => {
						if (err || stderr || !stdout) {
							logError(`go list failed on ${this.delve.program}: ${stderr || err}`);
							return resolve('');
						}
						resolve(stdout.trim());
					}
			...
			this.localGoModNameMapping.set(dirName, localGoModName);
		} catch (error) {
			logError(error);
			this.localGoModNameMapping.set(dirName, '');
		}
		return this.localGoModNameMapping.get(dirName);
	}
	...
	private async getPackageInfo(debugState: DebuggerState): Promise<string> {
		...
		const dir = path.dirname(
			this.delve.remotePath.length || this.delve.isRemoteDebugging
				? await this.toLocalPath(debugState.currentThread.file)
	...
	private async dumpStacktrace() {
		...
		this.delve.call<DebugLocation[] | StacktraceOut>(
			...
			async (err, out) => {
				...
				const locations = this.delve.isApiV1 ? <DebugLocation[]>out : (<StacktraceOut>out).Locations;
				log('locations', locations);
				const stackFrames = await Promise.all(locations.map(async (location, frameId) => {
					const uniqueStackFrameId = this.stackFrameHandles.create([goroutineId, frameId]);
					return new StackFrame(
						uniqueStackFrameId,
						location.function ? location.function.name : '<unknown>',
